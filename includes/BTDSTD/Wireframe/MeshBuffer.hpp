...
namespace Wireframe::MeshBuffers
{
	...
	//defines a multi-mesh buffer that interleaves index and vertex data
	struct MegaMeshBuffer
	{
		BTD::Map::IDRegistery IDRegistery; //the IDs associated with each index buffer

		//the total vertex data

		//the several index data arrays

		//the mega vertex buffer

		//the several index buffers

		//adds a vertex and index buffer, returning a ID
		inline uint64_t AddMeshData(const void* vertexData, const size_t& _vertexCount,
			const uint32_t* indicesData, const size_t& _indicesCount)
		{
